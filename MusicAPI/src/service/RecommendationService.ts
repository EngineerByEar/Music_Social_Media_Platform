...
export class RecommendationService {
    ...
    static async get_guest_recommendations(){
       const query = await DB.query(
           `SELECT p.post_id, p.post_title, u.username as post_author_username, pf.post_preview_image_url, pf.post_audio_url, count(w.post_id) as post_views_count, count(l.post_id) as post_likes_count
           FROM posts p

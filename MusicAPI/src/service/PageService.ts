...
import {IHistoryQuery, IPostsQuery, IProfileDataQuery} from "../model/PageModel.js";



export class PageService {

    static async load_profile(profile_id: number, requesting_username: string|undefined){

        const [ profile_data_rows ] = await DB.query<IProfileDataQuery[]>(`
            Select u.username,
            ...
            Where u.user_id = ?
            `,[profile_id])
        const profile_data = profile_data_rows[0] as IProfileDataQuery;


        const [post_previews] = await DB.query<IPostsQuery[]>(`
            Select p.post_id,
                   p.post_title,
    ...
    static async load_history(user_id: number){
        const [ history ] = await DB.query<IHistoryQuery[]>(`
            SELECT p.post_id,
                   p.post_title,
                   u.username as post_author_username,
                   pf.post_preview_image_url,
